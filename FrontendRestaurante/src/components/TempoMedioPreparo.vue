...
const lanches = computed(() => {
  ...
  const selected = new Date(lastDate.value)

  lanchesData.value.forEach(l => {
    const data = new Date(l.data)
    const match =
      (filterType.value === "day" && l.data === lastDate.value) ||
...
const maxTempo = computed(() => lanches.value.length ? Math.max(...lanches.value.map(l => l.tempo)) : 0)

function exportCSV() {
  let csv = "Lanche,Tempo Médio (min),Período\n"
  lanches.value.forEach(l => csv += `${l.nome},${l.tempo},${filterType.value}\n`)
...
function exportExcel() {
  const ws = XLSX.utils.json_to_sheet(
    lanches.value.map(l => ({ Lanche: l.nome, "Tempo Médio (min)": l.tempo, Período: filterType.value }))
  )
    ...
    <div class="space-y-2">
      <div v-for="l in lanches" :key="l.nome" class="flex flex-col">
        ...
        <div class="w-full h-3 bg-gray-700 dark:bg-gray-900 rounded-full overflow-hidden">
          <div class="h-3 bg-blue-500 rounded-full" :style="{ width: maxTempo ? (l.tempo/maxTempo)*100+'%' : '0%' }"></div>
        </div>
      </div>
    </div>
  </div>
